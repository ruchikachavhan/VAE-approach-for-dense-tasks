...
class NYUv2(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = torch.from_numpy(np.moveaxis(cv2.resize(np.load(self.data_path + '/image/{:d}.npy'.format(index)), image_size), -1, 0))
        semantic = torch.from_numpy(cv2.resize(np.load(self.data_path + '/label/{:d}.npy'.format(index)), image_size))
        depth = torch.from_numpy(np.moveaxis(cv2.resize(np.load(self.data_path + '/depth/{:d}.npy'.format(index)), image_size), -1, 0))
        normal = torch.from_numpy(np.moveaxis(np.load(self.data_path + '/normal/{:d}.npy'.format(index)), -1, 0))
        depth = depth.unsqueeze(0)
        # print(image.shape, semantic.shape, depth.shape, normal.shape)
